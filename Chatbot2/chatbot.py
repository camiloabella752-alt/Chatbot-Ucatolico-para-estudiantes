...
def predict_class(sentence):
    ...
    # Devuelve algo como: [0.1, 0.7, 0.05, 0.15] → (70% para intent 2) "PROBABILIDAD"

    ERROR_THRESHOLD = 0.50
    # Definimos un umbral mínimo de confianza (50%)
    # Si la probabilidad es menor, lo descartamos

    results = [[i, r] for i, r in enumerate(res) if r > ERROR_THRESHOLD]
    # enumerate(res) recorre cada probabilidad junto con su índice.
    # if r > ERROR_THRESHOLD se queda solo con las probabilidades mayores al 25%.
    # Los guarda como pares [índice, probabilidad].

    results.sort(key=lambda x: x[1], reverse=True)
    # Ordena el arreglo desde el intent con mayor probabilidad al menor.Asi es mas probable que quede el primero.

    return_list = []
    # Preparamos una lista vacia para almacenar los resultados.

    if len(results) == 0:
        # Si no reconoce el mensaje → devolver un intent fuera de tema
        return [{'intent': 'out_of_scope', 'probability': '1.0'}]
        # Devolvemos el intent out_of_scope que es: "No entiendo esto/"No es de la universidad"

    # SI hay intents validos, los covertimos en formato legible
    for r in results:
        return_list.append({
            'intent': classes[r[0]],  # r[0] es el índice del intent.
            # classes[r[0]] obtiene el nombre del intent usando ese índice.
            'probability': str(r[1])  # r[1] es la probabilidad.
        })

    return return_list  # Entrega el intent ya aprobado
    # La función lo que hace es:

# 1.Procesa la frase.

# 2.El modelo predice los intents.

# 3.Filtra los que tengan buena probabilidad.

# 4.Si ninguno llega al nivel mínimo → devuelve "out_of_scope".

# 5.Si hay uno o más → devuelve el más probable con su nombre y probabilidad.
